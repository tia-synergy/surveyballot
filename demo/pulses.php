<?php
	...
	$selectHeartbeat = "select * from tb_heartbeat where client_id='$client_id' order by senddate desc";
			...
			<div class="row">
				...
				<div class="span8">
					...
					<ul class="sf-menu fixed" id="menu">
						...
						<li class="current"><a href="pulses.php">Survey</a></li>
							...
							<div class="row">
								...
								<!-- The time line -->
									<ul class="timeline">
										...
										{
											$senddate = date_parse($rowHeartbeat['senddate']);
											
											$selHeartbeatDetail = "select * from tb_customerheartbeat where heartbeat_id=".$rowHeartbeat['id']." and client_id='$client_id'";
											$resHeartbeatDetail = mysql_query($selHeartbeatDetail);
											
											$totalsend = mysql_num_rows($resHeartbeatDetail);
											$totalopened = 0;
											$totalResponse = 0;
											$questio1_response = 0;
											$questio2_response = 0;
											...
											while($rowHeartbeatDetail=mysql_fetch_array($resHeartbeatDetail))
											{
												if($rowHeartbeatDetail['status'] == 'emailopened')
												{
													$totalopened = $totalopened + 1;
												}
												elseif($rowHeartbeatDetail['status'] == 'responsereceived')
												{
													$totalopened = $totalopened + 1;
													$totalResponse = $totalResponse + 1;
													
													$questio1_response = $questio1_response + $rowHeartbeatDetail['questio1_response'];
													$questio2_response = $questio2_response + $rowHeartbeatDetail['questio2_response'];
													$questio3_response = $questio3_response + $rowHeartbeatDetail['questio3_response'];
													$questio4_response = $questio4_response + $rowHeartbeatDetail['questio4_response'];
												}
											}
											
											$openpercent = (100*$totalopened)/$totalsend;
											$responsepercent = (100*$totalResponse)/$totalsend;
										?>
										<!-- timeline item -->
										...
										<li>
											<div class="time-label">
												<span class="bg-green">
													<?php echo $senddate['day'].' '.get_month($senddate['month']).', '.$senddate['year'];?>
												</span>
											</div>
											<div class="timeline-item">
												<h3 class="timeline-header"><?php echo $totalResponse;?> of <?php echo $totalsend;?> survey received.</h3>
												<div class="timeline-body">
													<span><strong>Open rate: </strong><?php echo number_format($openpercent,2).'%';?></span>
													<span style="margin-left:10px;"><strong>Response rate: </strong><?php echo number_format($responsepercent,2).'%';?></span>
												</div>
											</div>
										</li>
										<!-- END timeline item -->
