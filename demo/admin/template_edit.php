<?php
	...
	$selTempQuestions = "select * from tb_templateQuestion where template_id='$template_id'";
	$resTempQuestions = mysql_query($selTempQuestions);
	...
	if($_SERVER['REQUEST_METHOD'] == "POST")
	{
		if(!empty($_POST['template-submit']))
		{
			...
			$update_query = "update tb_template set name='$t_name', email_sub='$email_sub', email_body='$email_body' where id='$template_id'";
			...
			for($i=1;$i<=10;$i++)
			{
				...
				$query = "update tb_templateQuestion set question_name='$question_name', question='$question', updatedate=NOW() where template_id='$template_id' and question_no='$question_no'";
...
<html>
	<head>
		<meta charset="UTF-8">
		<title>Survey Ballot Admin | Template View</title>
						...
						<div class="box-body">
							...
							<form role="form" id="template-form" name="template-form" method="post" action="<?php echo $_SERVER['REQUEST_URI'];?>">
								<div class="form-group">
									...
									<input type="text" class="form-control" id="t_name" name="t_name" value="<?php echo $rowTemplate['name'];?>">
								</div>
								<div class="form-group">
									<label for="email_sub">Email Subject</label>
									<input type="text" class="form-control" id="email_sub" name="email_sub" value="<?php echo $rowTemplate['email_sub'];?>" >
								</div>
								<div class="form-group">
									<label for="email_body">Email Body</label>
									<textarea class="textarea" id="email_body" name="email_body" placeholder="Place some text here" style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;"><?php echo $rowTemplate['email_body'];?></textarea>
								</div>
								<?php 
								while($rowTempQuestions=mysql_fetch_array($resTempQuestions))
								{
									$questionnameid = "question".$rowTempQuestions['question_no']."_name";
									$questionid = "question".$rowTempQuestions['question_no'];
								?>
									<div class="form-group">
										<label>Question <?php echo $rowTempQuestions['question_no'];?></label>
										<input class="form-control" name="<?php echo $questionnameid;?>" id="<?php echo $questionnameid;?>" placeholder="title" value="<?php echo $rowTempQuestions['question_name'];?>" />
										<textarea class="form-control" name="<?php echo $questionid;?>" id="<?php echo $questionid;?>" placeholder="question"><?php echo $rowTempQuestions['question'];?></textarea>
									</div>
								<?php
								}
								?>
