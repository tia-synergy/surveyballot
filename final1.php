<?php
 ...
 $id = $_REQUEST["token"];
	//$selheartbeat = "select * from tb_customerheartbeat where email_token='$token'"; echo $selheartbeat = "select * from tb_survey where id='".$id."'";
	$resheartbeat = mysql_query($selheartbeat);
	$rowheartbeat = mysql_fetch_array($resheartbeat);
	$token=rowheartbeat['token'];
	...
	*/	echo $customer_id = $rowheartbeat['customer_id'];
	echo $selcustomer = "select * from tb_customer where id='$customer_id'";
	...
	*/
	...
	{
		if(!empty($_POST['testimonial-submit']))
		{
			// print_r($_POST);
			$testimonial = $_POST['testimonial'];
			
			$updateHeartbeat = "update tb_customerheartbeat set status='responsereceived', response_date=NOW(), question1_response='$question1_response', question2_response='$question2_response', question3_response='$question3_response', question4_response='$question4_response', overall_response='$overall_response', recommend_response='$recommend_response' where id='$token'";
			$resUpdate = mysql_query($updateHeartbeat);
			
			if($recommend_response==1)
			{
				header("location:testimonial.php?id='$customer_id'");
			}
		}
	}
?>
				...
				<form class="fixed" id="testimonial-form" name="testimonial-form" method="post" action="<?php echo $_SERVER['REQUEST_URI'];?>">
					<div>
						<div align="center" style="padding-top:40px;">
							...
							<h5 style="padding-bottom:10px;">Thank you for taking time to participate in our survey. We truly value the information you have provided. By participating in this survey, </br>you made your voice heard and are helping shape the future of Synergy IT Solutions Inc. </h5><h5 style="padding-bottom:30px;"> <!--style="text-align:justify"-->Thank you again for your time and input,</h5><h5 > <!--style="text-align:justify;padding-top:40px;"-->Sincerely,<br/>Kiran Gill</br>Synergy IT Solutions Inc.</h5>
						</div>
					</div>
				</form>
			</div><!-- end .row -->
